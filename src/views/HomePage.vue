...
import { 
  setConnection, 
  controlJob,
  sendHomeCommand,  // <-- 1. IMPORTAR
  sendLevelCommand  // <-- 2. IMPORTAR
} from '../services/api';

// --- Lógica da Câmara ---
const PI_IP_ADDRESS = '192.168.1.23'; // (Confirme o IP)
const streamUrl = ref(`http://${PI_IP_ADDRESS}/webcam/?action=stream`);
const streamError = ref(false);
function onStreamError() {
  console.error("Falha ao carregar o stream da câmara em: " + streamUrl.value);
...
// --- Lógica de Controlo de Impressão ---
...
// (isOperacional só é verdadeiro se for 'Operacional' e não 'Printing', etc.)
...
async function handlePauseResume() {
  ...
  try { await controlJob('pause'); } 
  catch (error) { alert("Erro ao enviar comando de pausa/retoma."); } 
  finally { setTimeout(() => { isJobLoading.value = false; }, 2000); }
}
...
// --- 3. NOVA LÓGICA: AÇÕES DA IMPRESSORA ---
...
function formatTime(seconds: number): string {
  ...
  if (parts.length === 0) return 'Calculando...';
  return parts.join(' ');
}
</script>
...
/* Os widgets de temperatura foram removidos */
...
.video-feed { 
  ...
  height: 100%; 
  object-fit: contain; 
  transform: rotate(180deg); /* <-- AQUI ESTÁ A CORREÇÃO */
}
...
.status-dot.Operacional { background-color: #28a745; animation: pulse-green 2s infinite; }
.status-dot.Printing { background-color: #007bff; animation: pulse-blue 2s infinite; }
.status-dot.Paused { background-color: #ffc107; animation: pulse-yellow 2s infinite; }
...
.btn-connect.btn-green { background-color: #28a745; }
.btn-connect.btn-green:hover { background-color: #218838; }
.btn-connect.btn-red { background-color: #dc3545; }
...
.btn-job.btn-pause { background-color: #ffc107; color: #212529; }
.btn-job.btn-pause:hover { background-color: #e0a800; }
.btn-job.btn-cancel { background-color: #dc3545; }
...
/* --- 4. NOVOS ESTILOS PARA OS BOTÕES DE AÇÃO --- */
...
.btn-action {
  ...
  font-weight: bold;
  color: #fff;
  background-color: var(--icon-active); /* Azul padrão */
  border: none;
  border-radius: 8px;
  cursor: pointer;
